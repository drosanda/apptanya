...
class Seme_Email
{
  ...
  */
  public function to($mail, $name="")
  {
    if (strlen($name ?? '') > 0) {
      ...
      $this->to[] = $mail;
      $this->toname[] = "";
      $this->log .= "Send to $mail".$this->eol;
    }
    return $this;
  }
  ...
  */
  public function template($template)
  {
    ...
    $basetemp = __DIR__.'/seme_email/';
    if (!is_dir($basetemp)) {
      mkdir($basetemp);
    }
    $basetemp = __DIR__.'/seme_email/template/';
    if (!is_dir($basetemp)) {
      mkdir($basetemp);
    }
  ...
  */
  ...
  public function preview(){
    $mail = $this->_process();
    echo '<html><head><title>Email Preview</title></title></head><body>';
    foreach ($this->to as $mail) {
      echo '<div style="padding: 1em; background-color: #ededed;margin-bottom: 1em;">';
      echo '<p><b>Subject: '.$this->subject.'</b></p>';
      echo '<p><b>To: '.$mail.'</b></p>';
      echo '</div>';
