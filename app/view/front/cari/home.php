<div class="section">
  <div class="column">
    <div class="content">
      <h1 class="text-center">Cari pertanyaan / Jawaban</h1>
      <form id="fcari" method="get" action="<?=base_url('cari/')?>" class="form-horizontal">
        <div class="form-group">
          <input id="ikeyword" type="text" class="form-control" name="keyword" placeholder="cth: dimana beli lele" minlength="1" required />
        </div>
        <div class="form-group form-action">
          <button type="submit" class="btn btn-primary btn-submit">Lakukan Pencarian <i class="icon-submit fa fa-search"></i></button>
        </div>
      </form>
    </div>
  </div>
</div>
